import {
  ...
  async function getDogContest(gameId) {
    if (!client) return;
  ...
  async function getDogShareImage({ catImageUrl, score, place, nick }) {
    const res = await fetch('/api/contest/share-preview-dog', {
      method: 'POST',
  ...
  async function dogBeauty(picture, nick, id) {
    if (!client) return;
    const submitBtn = document.getElementById('saveBtn');
    const progress = document.getElementById('saveProgress');
    if (submitBtn) submitBtn.classList.add('hidden');
      ...
      case 1:
        ...
        if (!players.length) {
          ...
          if (statusContainer && game.game_active === 'True') {
            ...
            joinBtn.textContent = 'Show your dog';
            joinBtn.addEventListener('click', () => {
              window.location.href = '/dog-beauty';
            });
        ...
        if (statusContainer) {
          ...
          if (meIndex >= 0) {
            ...
            msg.textContent = `Congrats, your dog is in ${place}${place === 1 ? 'st' : place === 2 ? 'nd' : place === 3 ? 'rd' : 'th'} place`;
            ...
            btn.type = 'button';
            btn.className = 'btn';
            btn.textContent = 'Change dog\'s photo';
            ...
            shareBtn.addEventListener('click', async () => {
              try {
                ...
                const url = await WalletUI.getDogShareImage({
                ...
                const text = `My dog is in #${place} place in Dog Beauty Contest “${game.game_title}”!`;
            ...
            const msg = document.createElement('div');
            if (game.game_active === 'True') {
              msg.textContent = 'Your dog is not participating in the contest yet.';
            } else {
              msg.textContent = 'Your dog did not participate in the contest.';
            }
            msg.style.marginBottom = '.5rem';
        ...
        sorted.forEach((p, index) => {
          ...
          img.addEventListener('click', () => openPhoto(p.dog));
          ...
          catName.textContent = p.dog_nick || 'Unnamed dog';
